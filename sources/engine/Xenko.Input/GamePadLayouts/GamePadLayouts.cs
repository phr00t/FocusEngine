    ...
    public static class GamePadLayouts
    {
        ...
        private static void LoadGenericSDLLayouts()
        {
            ...
            foreach (var line in contents)
            {
                ...
                if (properties.TryGetValue("platform", out string controllerPlatform) && CompatibleWithCurrentPlatform(controllerPlatform))
        ...
        private static bool TryParseSDLGuid(string input, out Guid guid)
        {
            ...
            {
                if (!byte.TryParse(input.AsSpan(i, 2).ToString(), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var b))
                    return false;
                bytes[i / 2] = b;
            }
