...
namespace Xenko.UI
{
    ...
    public abstract partial class UIElement : IUIElementUpdate, IUIElementChildren, IIdentifiable
    {
        ...
        protected internal virtual bool Intersects(ref Ray ray, out Vector3 intersectionPoint, bool nonUISpace, Vector2? canvasScale = null)
        {
            // does ray intersect element Oxy face?
            bool intersects;

            // resize canvas?
            Vector3 scaledRenderSize = RenderSizeInternal;
            if (canvasScale.HasValue)
            {
                scaledRenderSize.X *= canvasScale.Value.X;
                scaledRenderSize.Y *= canvasScale.Value.Y;
            }
            ...
            if (nonUISpace)
            {
                ...
                float rsx = scaledRenderSize.X * 0.5f;
                float rsy = scaledRenderSize.Y * 0.5f;
            ...
            {
                intersects = CollisionHelper.RayIntersectsRectangle(ref ray, ref WorldMatrixPickingInternal, ref scaledRenderSize, 2, out intersectionPoint);
            }
